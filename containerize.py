...
def optimal(vertex_num, tasks, processors, d, r_dag, order):
    ...
    for combination in itertools.product(*([[0,1,2,3] for _ in range(len(tasks)-1)])):
        ...
        new_tasks, com = update_schedule(d, r_dag, processors, tasks, bridge_tasks, order, cont_set)
...
def containerize(tasks, processors, d, dag, r_dag, index, t, N, order, flag, Graph = example.graph):
    ...
    new_tasks, bridge_com = update_schedule(d, r_dag, processors, tasks, bridge_tasks, order, cont_set)

    return cont, bridge_tasks, new_tasks, bridge_com
