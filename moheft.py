...
def schedule(task):
    global solutions
    new_solutions = []
    heapq.heapify(new_solutions)
    for (tasks, processors) in solutions:
        for p in processors:
            new_tasks, new_processors = copy.deepcopy(tasks), copy.deepcopy(processors)
            assign(task.id, p.id, new_tasks, new_processors)
            m = max(p.avail for p in new_processors)
            heapq.heappush(new_solutions, (m+random.random()*0.0001, (new_tasks, new_processors))) # random number is the tie breaker

    solutions = []
    if len(solutions) < K:
        solutions.append(itemgetter(1)(heapq.heappop(new_solutions)))
...
def moheft():
    ...
    solutions.append((copy.deepcopy(tasks), copy.deepcopy(processors)))

    for task in priority_list:
        schedule(task)

    tasks = itemgetter(0)(solutions[0])
    processors = itemgetter(1)(solutions[0])
