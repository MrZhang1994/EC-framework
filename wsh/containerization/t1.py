...
if __name__ == '__main__':
    try:
        length = 1000
        file = open('/root/runtime/t1_s.log', 'w')
        # pass-in arguments
        host = argv[1]
        port = int(argv[2])
        ...
        client.connect()
        client2 = JsonClient('worker-2', port)
        client2.connect()
        client3 = JsonClient('worker-3', port)
        client3.connect()
        client.send_obj(data_to_send, client.socket)
        client2.send_obj(data_to_send, client2.socket)
        client3.send_obj(data_to_send, client3.socket)
        client.close()
        client2.close()
        client3.close()
